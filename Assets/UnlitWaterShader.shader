Shader "Unlit/UnlitWaterShader"
{
	...
	_NormalMap2("Normal Map2", 2D) = "blue" {}
	}
		SubShader
	{
		Tags{
...
#include "UnityCG.cginc"
	...
	v2f vert(appdata v)
	{
		...
		o.vertex.x += sin(worldPos.z + _Time.x * 10) * 0.1;
		o.vertex.y += sin(worldPos.x + _Time.x * 10) * 0.1;
		o.vertex.z += sin(worldPos.y + _Time.x * 10) * 0.1;
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		float3 norm2 = UnpackNormal(tex2D(_NormalMap2, i.uv + half2(0, _Time.x)));
		//offset object normal RG with texture RG
		i.objectNormal.rg += (norm.rg + norm2.rg) * .33;
		//i.objectNormal.rg += norm2.rg * .5;
